...
// ── Health check mode (для Docker HEALTHCHECK) ────────────────────────────────
...
{
    e.Cancel = true;
    Console.WriteLine("\n  [Graceful shutdown] Отримано сигнал зупинки...");
    cts.Cancel();
};

AppDomain.CurrentDomain.ProcessExit += (_, _) =>
{
    Console.WriteLine("  [Graceful shutdown] Збереження стану перед виходом...");
    // Тут можна зберегти стан у /app/data/state.json
    var statePath = Path.Combine(AppContext.BaseDirectory, "data", "shutdown.log");
    Directory.CreateDirectory(Path.GetDirectoryName(statePath)!);
    File.AppendAllText(statePath, $"{DateTime.UtcNow:O} — застосунок зупинено{Environment.NewLine}");
};
...
void RunMenu(CancellationToken token)
{
    ...
    Console.WriteLine("  Застосунок завершено коректно.");
}
